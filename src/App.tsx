  ...
  return (
    <div className="relative w-screen h-screen bg-[#05070a] overflow-hidden font-sans select-none">
      ...
      <header className="absolute top-0 left-0 right-0 z-10 p-3 sm:p-4 pointer-events-none flex justify-between items-center">
        {/* Title & Mode Badge & Quick Weapon Selector */}
        <div className="pointer-events-auto flex items-center gap-2 bg-black/60 backdrop-blur-md border border-white/10 px-3 py-1.5 rounded-xl shadow-md">
          ...
          <span className="text-xs font-black text-white tracking-wider uppercase font-mono hidden sm:inline">
            FLORESTA TÁTICA
          </span>

          <div className="flex items-center gap-1 font-mono text-[10px] pl-2 border-l border-white/10">
            {[
              { id: 'ak47', label: '1: AK-47' },
              { id: 'm4a1', label: '2: M4A1' },
              { id: 'm92fs', label: '3: M92FS' },
              { id: 'mp5', label: '4: MP5' },
            ].map((w) => (
              <button
                key={w.id}
                onClick={() => engineRef.current?.switchWeapon(w.id)}
                className={`px-2 py-0.5 rounded font-bold uppercase transition-all cursor-pointer ${
                  fpsState.weapon.id === w.id
                    ? 'bg-red-600 text-white border border-red-400/50 shadow'
      ...
      {!fpsState.weapon.isAiming && (
        <div className="absolute inset-0 pointer-events-none z-20 flex items-center justify-center">
          <div className="relative flex items-center justify-center">
            ...
            {(() => {
              const spreadPx = Math.max(6, Math.min(36, Math.round(fpsState.weapon.currentSpreadRad * 950)));
              const isHighSpread = spreadPx > 14;
              const tickColorClass = isHighSpread ? 'bg-red-400' : 'bg-white/90';

              return (
                <>
                  <div
                    style={{ transform: `translateY(-${spreadPx}px)` }}
                    className={`absolute w-0.5 h-2 ${tickColorClass} transition-transform duration-75 ease-out shadow-sm`}
                  />
                  <div
                    style={{ transform: `translateY(${spreadPx}px)` }}
                    className={`absolute w-0.5 h-2 ${tickColorClass} transition-transform duration-75 ease-out shadow-sm`}
                  />
                  <div
                    style={{ transform: `translateX(-${spreadPx}px)` }}
                    className={`absolute h-0.5 w-2 ${tickColorClass} transition-transform duration-75 ease-out shadow-sm`}
                  />
                  <div
                    style={{ transform: `translateX(${spreadPx}px)` }}
                    className={`absolute h-0.5 w-2 ${tickColorClass} transition-transform duration-75 ease-out shadow-sm`}
      ...
      {/* Bottom Right Tactical Weapon & Ammo HUD */}
        ...
        <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-xl px-3 py-1.5 flex items-center gap-3 text-slate-200 shadow-lg">
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-red-400 font-black uppercase tracking-wider">{fpsState.weapon.name}</span>
              <span className="text-[8px] bg-red-500/20 text-red-300 border border-red-400/30 px-1 py-0.2 rounded font-bold">{fpsState.weapon.caliber}</span>
            </div>
