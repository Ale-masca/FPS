...
export class FPSViewModel {
  ...
  // Physical Spring Recoil Velocities
  private recoilPitchVel = 0;
  private recoilYawVel = 0;
  private recoilRollVel = 0;
  private recoilZVel = 0;
  private recoilYVel = 0;
  ...
  constructor(camera: THREE.Camera, options: ViewModelOptions = {}) {
    ...
    // Ensure all ViewModel meshes belong to Layer 1 for dedicated camera fill light
    this.enableViewModelLayers();
  }

  public enableViewModelLayers() {
  ...
  // Trigger Recoil Kick Impulse (Physics-Based Instant Kick + Spring Velocity)
  public applyRecoil(recoilConfig?: WeaponRecoilConfig, burstCount: number = 0) {
    const vert = recoilConfig ? recoilConfig.vertical : 0.035;
    const horizVar = recoilConfig ? recoilConfig.horizontalVariance : 0.012;
    const horizBias = recoilConfig ? recoilConfig.horizontalBias : 0.08;
    const rollVar = recoilConfig ? recoilConfig.rollVariance : 0.01;
    const kickZ = recoilConfig ? recoilConfig.kickZ : 0.035;
    const kickY = recoilConfig ? recoilConfig.kickY : 0.01;
    const accumRate = recoilConfig ? recoilConfig.accumulationRate : 0.06;
    const maxAccum = recoilConfig ? recoilConfig.maxAccumulation : 1.5;

    const accum = Math.min(maxAccum, 1.0 + burstCount * accumRate);

    // Instant physical displacement punch on firing frame
    this.recoilPitch += vert * accum;
    this.recoilYaw += (horizBias + (Math.random() - 0.5) * 2 * horizVar) * accum;
    this.recoilRoll += (Math.random() - 0.5) * 2 * rollVar * accum;
    this.recoilZ += kickZ * accum;
    this.recoilY += kickY * accum;

    // Apply punchy spring velocity impulse for rapid spring recoil return
    const impulseScale = 14.0;
    this.recoilPitchVel += vert * accum * impulseScale;
    this.recoilYawVel += (horizBias + (Math.random() - 0.5) * 2 * horizVar) * accum * impulseScale;
    this.recoilRollVel += (Math.random() - 0.5) * 2 * rollVar * accum * impulseScale;
    this.recoilZVel += kickZ * accum * impulseScale;
    this.recoilYVel += kickY * accum * impulseScale;

    this.containerScaleZ = 0.85 * 0.96;

    if (this.weaponModel) {
      this.weaponModel.muzzleFlashLight.intensity = 8.5;
      this.weaponModel.muzzleFlashMesh.visible = true;
      this.weaponModel.muzzleFlashMesh.rotation.z = Math.random() * Math.PI * 2;
    }
  }
  ...
  public update(
    ...
    isGrounded: boolean,
    leanRoll: number = 0,
    recoilConfig?: WeaponRecoilConfig
  ) {
    // 1. Damped Spring Physics Integration for ViewModel Recoil
    const recoverSpeed = recoilConfig ? recoilConfig.recoverSpeed : 22.0;
    const damping = recoilConfig ? recoilConfig.damping : 14.0;
    const stiffness = recoverSpeed * recoverSpeed;

    // Pitch Spring
    const pitchAccel = -stiffness * this.recoilPitch - damping * this.recoilPitchVel;
    this.recoilPitchVel += pitchAccel * delta;
    this.recoilPitch += this.recoilPitchVel * delta;

    // Yaw Spring
    const yawAccel = -stiffness * this.recoilYaw - damping * this.recoilYawVel;
    this.recoilYawVel += yawAccel * delta;
    this.recoilYaw += this.recoilYawVel * delta;

    // Roll Spring
    const rollAccel = -stiffness * this.recoilRoll - damping * this.recoilRollVel;
    this.recoilRollVel += rollAccel * delta;
    this.recoilRoll += this.recoilRollVel * delta;

    // Z Translation Kick Spring
    const zAccel = -stiffness * this.recoilZ - damping * this.recoilZVel;
    this.recoilZVel += zAccel * delta;
    this.recoilZ += this.recoilZVel * delta;

    // Y Translation Kick Spring
    const yAccel = -stiffness * this.recoilY - damping * this.recoilYVel;
    this.recoilYVel += yAccel * delta;
    this.recoilY += this.recoilYVel * delta;
    ...
    // Recoil ADS scaling (keep optics readable during aiming down sights)
    const recoilAdsScale = 1.0 - this.adsWeight * 0.45;

    this.rootGroup.position.set(
      this.currentPos.x + this.currentSway.x + bobX * adsDamp,
      this.currentPos.y + this.currentSway.y + (this.recoilY * recoilAdsScale) - bobY * adsDamp + breathY,
      this.currentPos.z - (this.recoilZ * recoilAdsScale) - bobZ * adsDamp
    );
    ...
    // Total Rotations: Direct application of spring recoil for crisp FPS feel
    const rotX = -(this.recoilPitch * recoilAdsScale) + this.currentSway.y + inspectRotationX + reloadRotX + (moveRotPitch + staticSprintPitch) * adsDamp;
    const rotY = (this.recoilYaw * recoilAdsScale) + this.currentSway.x + inspectRotationY + (moveRotYaw + staticSprintYaw) * adsDamp;
    const rotZ = (this.recoilRoll * recoilAdsScale) + inspectRotationZ + reloadRotZ - leanRoll * 0.35 + (moveRotRoll + staticSprintRoll) * adsDamp;

    this.recoilGroup.rotation.set(rotX, rotY, rotZ);
