...
const ColorCorrectionShader = {
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // 1. Exposure multiplier & soft shadow toe lift for indirect GI detail retention
      color = color * (exposure * 1.35) + vec3(0.025);

      // 2. ACES Filmic Tone Mapping
      color = ACESFilm(color);

      // 3. Contrast adjustment (centered around mid-gray 0.5) with shadow floor protection
      ...
      // 5. Gamma Correction & sRGB Output Encoding (sRGB gamma ~2.2 curve)
...
export class PostProcessingManager {
  ...
  public settings: PostProcessingSettings = {
    contrast: 0.98,       // Soft balanced contrast preserving shadow details
    saturation: 1.06,     // Natural foliage color saturation
    exposure: 1.20,       // Bright atmospheric exposure
    gamma: 1.0,
    bloomStrength: 0.18,  // Subtle realistic bloom intensity
    bloomThreshold: 0.88, // Affects bright sky and highlights
    bloomRadius: 0.35,    // Soft localized bloom radius
    fogDensity: 0.0048,   // Gentle exponential distance fog density
    fogColor: '#94a3b8',  // Atmospheric sky fog tint
  };
